...
export const api = {
  ...
  relationships: {
    list: (personId) => request(personId ? `/relationships?person_id=${personId}` : '/relationships'),
    create: (body) => request('/relationships', { method: 'POST', body: JSON.stringify(body) }),
    remove: (id) => request(`/relationships/${id}`, { method: 'DELETE' }),
  },
  clips: {
    list: (personId) => request(personId ? `/clips?person_id=${personId}` : '/clips'),
    ...
    update: (id, body) => request(`/clips/${id}`, { method: 'PUT', body: JSON.stringify(body) }),
